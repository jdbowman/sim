...
Modifications (Leonid Benkevitch):
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  /*
   * At this point, the brightness image has been read from fits file
   * The image address is in double *data.
   */
  ...
  /* shift Y */
  for (i=0; i<n_cols; i++) {
    int halfaxis = n_cols/2;    // Maybe, = n_rows/2 ??????????????????????????
  ...
  /*
   * This is only executed when the sources are specified,
   * i.e. the command line option -s sources_filename is provided. 
   */ 
  if ( num_src > 0 ) {
  ...
  /*
   * First two numbers in the binary UV image are its dimensions
   */ 
  result = fwrite(imgsize, sizeof(int), 2, outfile); /* =========>>> OUTFILE */
  ...
  for(j = 0; j < n_rows; j++) {
	...
	if (i < icol1 || i >= icol2) {
     ...
     /*
      * Save a j-th row of the UV image in the binary file
      */
	...
	fprintf(stderr, "ERROR: wrote %d dcomplex, expected to write %ld\n",
      ...
      /* for(i1 = 0; i1 < icol1; i1++) /\* Save left half in ASCII *\/ */
      /* 	fprintf(fh, "%g %g ", *(mirror+i1),  *(mirror+i1+1)); */

      /* Right half ... */ 
      result = fwrite(mirror+mshift, 2*sizeof(double), icol1, outfile); /*>>*/
      if (result != icol1) {
	fprintf(stderr, "ERROR: wrote %d dcomplex, expected to write %ld\n",
		result, icol1);
      }
      /* for(i1 = 0; i1 < icol1; i1++)  /\* Save right half in ASCII *\/ */
      /* 	fprintf(fh, "%g %g ", *(mirror+mshift+i1), */  
      /*                              *(mirror+mshift+i1+1)); */
      /* fprintf(fh, "\n"); /\* EOL *\/ */
    }
  } /* for(j = 0; j < n_rows; j++)  */
...
static void im2uv_help() {
  printf("Usage: MAPS_im2uv -i infile -o outfile [OPTIONS]...\n" \
	 ...
	 "-n, --normalizer\t multiplicative constant to \n" \

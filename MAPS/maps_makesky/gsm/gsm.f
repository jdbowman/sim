! Angelica de Oliveira-Costa & Max Tegmark 2007
	...
	character*800 infile, outfile, gsmdir, root
	call getenv('GSMDIR',gsmdir);
	...
	if (ncomploaded.ne.ncomp) stop 
	call ComputeComponents(nu,ncomp,f)
	norm = f(ncomp+1)
	root = '/component_maps_408locked.dat'
	infile = gsmdir(1:lnblnk(gsmdir))//root
	print *,'Component file_____________________________',infile(1:lnblnk(infile))
	...
	character*800 infile, comline, gsmdir,root
	call getenv('GSMDIR',gsmdir)
	!
	root = '/components.dat'
	infile = gsmdir(1:lnblnk(gsmdir))//root
...
777	stop 'DEATH ERROR 2 COUNTING COLUMNS'	
	...
	common/PCA/x, y, ypp, n
	integer i
	real*8 a(ncompmax+1), nu, lnnu 
	lnnu = log(nu)
	do i=1,ncomp+1
	  call mysplint_r8(x,y(1,i),ypp(1,i),n,lnnu,a(i))
  	  print *,'component ', i, ' is ' , a(i)
	end do

...
*   $Id: HPXcvt.c,v 0.0.1.7 2007/08/02 05:22:45 cal103 Exp cal103 $
*=============================================================================
*
...
char usage[] =
"Usage: HPXcvt [-c<sys>] [-n|-r] [-q<quad>] [-x<n|s>] [-t<hdu table>] [-b<binary column>] [<infile> "
"[<outfile>]]\n"
"\n"
...
"\n"
"Options:\n"
"  -b<col>	Column in binary table to read\n"
...
"  -t<table>	increment HDU this many from start.\n";
...
int HEALPixIn(struct healpix *hpxdat, int input_table,int bincol);
...

{
  ...
  if ((status = HEALPixIn(&hpxdat, input_table,bincol))) {
    return 2;
  }
...

{
  ...
  if (fits_movrel_hdu(fptr,input_table,&hdutype,&status)) goto fitserr;
  ...
  if (hdutype == IMAGE_HDU) {
    ...
    /* The data is assumed to be in column 1.  (Note that fits_get_coltypell()
     * is not available in cfitsio 2.x.) */
    if (fits_get_coltype(fptr, bincol, 0x0, &repeat, 0x0, &status)) goto fitserr;
  ...
  if (hdutype == IMAGE_HDU) {
    ...
    for (irow = 0; irow < nrow; irow++) {
      if (fits_read_col_flt(fptr, bincol, irow+1, firstelem, nelem, nulval, datap,
          &anynul, &status)) goto fitserr;
      datap += nelem;
    }
  }
...

{
  ...
  char  history[72];
  int   facet, halve, i, i1, i2, ifacet, j, jfacet, layout, nfacet, nside,
        rotn, status;

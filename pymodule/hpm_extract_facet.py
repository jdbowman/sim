...
class Img:
    ...
    def get_LM(self, center=(0, 0)):
        ...
        dim = self.shape
        M, L = n.indices(dim)
        L, M = n.where(L > dim[1] / 2, -(L-dim[1]/2), (dim[1]/2 - L)),\
            n.where(M > dim[0]/2, (M-dim[0]/2), -(dim[0]/2 - M))
...
if opts.osys == 'ga':
    ...
    for src in opts.src.split(','):
        cat[src] = ephem.Galactic(float(src.split('_')[0])*a.img.deg2rad,
                                  float(src.split('_')[1])*a.img.deg2rad)
...
if opts.osys != 'ga':
    ...
    for s in cat.values():
        try:
            ...
            a.phs.RadioSpecial.compute(s, o)
#infile = args[0]
for infile in args:
    ...
    for name, s in cat.iteritems():
       ...
       #     print RA,DEC
        ...
        if opts.output_weights:
            ...
            to_fits(wgt_filename, wgt_img, s, name,
                    history='hpm_extract_facet:  Wgts for the tacet at %s \
                    extracted from healpix map %s [%s]'
                    % (name, infile, time.asctime()))
        img.shape = im.shape
        filename = infile[:-len('.fits')] + '_' + name + '.fits'
        to_fits(filename, img, s, name,
                history='hpm_extract_facet:  Facet at %s extracted from \
                healpix map %s [%s]' % (name, infile, time.asctime()))